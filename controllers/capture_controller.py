...
from models.create_keypoints import create_keypoints
...
def capture_new_sign(input):
    ...
    new_sample_created = model_capture_sign(dir_word_path)
    if not new_sample_created:
        estract_kp = create_keypoints(dir_word_path)
        if estract_kp:
